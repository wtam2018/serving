...
// MakeClusterIngressTLS creates IngressTLS to configure the ingress TLS.
func MakeClusterIngressTLS(cert *v1alpha1.Certificate, hostNames []string) v1alpha1.IngressTLS {
...
// MakeIngress creates Ingress to set up routing rules. Such Ingress specifies which Hosts
// that it applies to, as well as the routing rules.
func MakeIngress(ctx context.Context, r *servingv1alpha1.Route, tc *traffic.Config, tls []v1alpha1.IngressTLS, ingressClass string) (*v1alpha1.Ingress, error) {
	...
	return &v1alpha1.Ingress{
		ObjectMeta: metav1.ObjectMeta{
			// As Ingress resource is cluster-scoped,
			// here we use GenerateName to avoid conflict.
