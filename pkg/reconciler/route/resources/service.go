...
// MakeIgressK8sService creates a Service that redirect to the loadbalancer specified
// in ClusterIngress status. It's owned by the provided v1alpha1.Route.
// The purpose of this service is to provide a domain name for Istio routing.
func MakeIgressK8sService(route *v1alpha1.Route, targetName string, ingress *netv1alpha1.Ingress) (*corev1.Service, error) {
	svcSpec, err := makeIngressServiceSpec(ingress)
	if err != nil {
		return nil, err
	}
...
func makeIngressServiceSpec(ingress *netv1alpha1.Ingress) (*corev1.ServiceSpec, error) {
	ingressStatus := ingress.Status
	if ingressStatus.LoadBalancer == nil || len(ingressStatus.LoadBalancer.Ingress) == 0 {
		return nil, errLoadBalancerNotFound
	}
